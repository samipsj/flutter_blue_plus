...
class FlutterBluePlus {
  ...
  /// Retrieve a list of devices currently connected from identifiers
  /// -
  /// - You must still call device.connect() to connect them to *your app*
  /// - only for iOS
  static Future<List<BluetoothDevice>> retrievePeripherals(List<String> identifiers) async {
    var r = await _invokeMethod(() =>
        FlutterBluePlusPlatform.instance.retrievePeripherals(BmRetrievePeripheralsRequest(identifiers: identifiers)));
    for (BmBluetoothDevice device in r.devices) {
      if (device.platformName != null) {
        _platformNames[device.remoteId] = device.platformName!;
      }
